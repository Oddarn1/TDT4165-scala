...
object Main {
  ...
  * one or both counters has been ran. This is a race condition, and is a problem when executing
  * multiple operations at once where you have no way to tell which is executed first, and the order is important.*/
  def ThreadedCounter(safe:Boolean):Unit={
    ...
    c1.start()
    //Thread.sleep to provoke a race condition
    c2.start()
    p.start()
  }
  ...
  def main(args: Array[String]): Unit = {
    print("Threaded counter(try running a couple times):")
    ThreadedCounter(safe=false)
    Thread.sleep(500)
    print("\nThreadsafe counter:")
    ThreadedCounter(safe=true)
    Thread.sleep(500)
    print("\nIf nothing happens for 10 seconds, there is a deadlock, program will crash. If no deadlock, try again.")
    DeadLockExample()
  }
}
